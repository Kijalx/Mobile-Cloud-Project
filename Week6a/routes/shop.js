...
router.post("/product/add", (req, res) => {
    ...
    new Product({ name, price })
        .save()
        .then((product) => { // 'product' contains the saved product, including its _id and any default values set by your schema
            console.log("Product added successfully");
            res.json(product); // Send back the newly created product
        })
        .catch((err) => {
...
router.get("/", (req, res) => {
    Product.find()
        .then((products) => {
            ...
            responseHtml +=
                '<a href="/addProduct"><button>Add New Product</button></a><br/><br/>'; // Add Product button
            responseHtml += "<ul>";
            // List each product as a clickable link for editing
...
router.get("/updateProduct/:id", (req, res) => {
    ...
    Product.findById(id)
        .then((product) => {
            ...
            // Building the form for updating the product
